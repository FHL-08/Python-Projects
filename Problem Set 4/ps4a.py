...
def innum():
    try:
        HAND_SIZE = int(input("Number of letters: "))
        return HAND_SIZE 
    except:
        print("Invalid Entry.")
        return innum()
...
def getWordScore(word, n):
    ...
    """
    bonus = 0
    score_list=[SCRABBLE_LETTER_VALUES[let] for let in word.lower()]
    if n == len(word):
        bonus = 50
    return (sum(score_list)*len(word))+bonus
...
def updateHand(hand, word):
    ...
    returns: dictionary (string -> int)
    """
    return {key: value-(word.count(key)) for (key, value) in hand.items()}
